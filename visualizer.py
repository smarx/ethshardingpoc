...
import random
import hashlib
from simulator import watcher
from blocks import Block
from simulator import GENESIS_BLOCKS
...
for message in watcher.consensus_messages:
	...
	blockPos[block] = (block.height, SHARD_SPACING_CONSTANT*block.shard_ID + message.sender)
	if block.prevblock is not None and block.prevblock in blocks:
		BlocksGraph2.add_edge(block, block.prevblock)
	nx.draw_networkx_nodes(BlocksGraph2, blockPos, node_shape='o', node_color='b', node_size=10)
	nx.draw_networkx_edges(BlocksGraph2, blockPos) 
