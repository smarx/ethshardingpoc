...
from config import GENESIS_BLOCKS
...
class Validator:
    ...
    def receive_protocol_messages(messages):
        for m in messages:
    ...
    def get_weighted_blocks(self):
        weighted_blocks = {}
        L_M = latest_messages(self.consensus_messages)
        for v in VALIDATOR_NAMES:
    ...
    def make_block(self, shard_ID, data, TTL=TTL_CONSTANT):
        ...
        for ID in SHARD_IDS:
            ...
            for payload in new_outgoing_payloads[ID]:
                ...
                sent_log = prevblock.sent_log.add_sent_messages(new_sent_messages)
    ...
    def make_new_consensus_message(self, shard_ID, data, TTL=TTL_CONSTANT)
        ...
        new_block = make_block(self, shard_ID, data, TTL)
